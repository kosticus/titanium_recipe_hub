...
/**
 * Menu on that is deisplay when the menu button is clicked
 * @param {Function} menuClickEvent, if provided, it is used instead of the event listener
 * 
 * @method _reloadData();
 * 
...
function MenuView(menuClickEvent) {
	...
	var createRow = function(name) {
		...
		row.addEventListener('click', function(e) {
			...
			if (menuClickEvent) {
				menuClickEvent(eventObj);
				return;
			}
			// click event for ios
			self.fireEvent('MenuView:rowClickEvent', eventObj);
		});
		return row;
	};
	
	/**
	 * Creates a header view for the table section
	 * @param {String} text
	 */
	...
	/**
	 * Reloads the table view - this is needed if the user changes from logged in to not logged in
	 */
	self._reloadData = function() {
		table.setDate(getRowData());
	};
